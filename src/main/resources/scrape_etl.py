    ...
    for filename in os.listdir(os.path.join(base_dir, directory)):
        ...
        for broker in p[3:-5]:
            ...
            if (text.find("View More Results:") == -1 and text.find("MC #") != -1 ):
                ...
                output.write(("\"" + name.strip() + "\",\"" + mc.strip() + "\",\"" + dot.strip() + "\",\"" + city.strip() + "\",\"" + state.strip() + "\"").encode('utf-8').strip() + '\n')
                final_csv.write(("\"" + name.strip() + "\",\"" + mc.strip() + "\",\"" + dot.strip() + "\",\"" + city.strip() + "\",\"" + state.strip() + "\"").encode('utf-8').strip() + '\n')
